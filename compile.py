...
def run_once() -> dict:
    ...
    # Support multiple LLM backends — prefer LLM_BACKEND env over hardcoded Anthropic
    if LLM_BACKEND == "ollama":
        client = None  # ollama calls are made directly in compile functions
    elif LLM_BACKEND == "openai":
        # OpenAI-compatible backend (e.g. OpenRouter) — build a passthrough client
        try:
            import openai as _openai
            ...
            # Wrap in a minimal shim so compile_with_claude can use it
            client = _oai_client
        except ImportError:
        ...
        if not api_key:
            logger.error("ANTHROPIC_API_KEY not set (set LLM_BACKEND=ollama or LLM_BACKEND=openai as alternative)")
            sys.exit(1)
