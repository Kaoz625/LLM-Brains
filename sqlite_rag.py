...
#         log.info("sqlite-vec loaded — vector search enabled")
        ...
        pass  # sqlite-vec not installed — skipping vector index
...
def get_reranker():
    """Lazy-load the cross-encoder reranker. Returns None if unavailable."""
    global _reranker
    ...
    try:
        ...
        print(f"Reranker load error: {e}", file=sys.stderr)
        return None
...
def rerank_results(query: str, results: list[dict], top_k: int = 5) -> list[dict]:
    ...
    Pipeline:
      FTS5 / hybrid_search returns up to 20 candidates
    ...
    Falls back to the original ordering if the reranker is unavailable.
    """
    ...
    pairs = [
        (query, f"{r.get('title', '')} {r.get('content', '')[:400]}")
        for r in results
    ]
    try:
        ...
        print(f"Reranker predict error: {e}", file=sys.stderr)

    return results[:top_k]


def search_reranked(query: str, candidate_limit: int = 20, top_k: int = 5,
                    db_path: Path = DB_PATH) -> list[dict]:
    """
    Full reranked search pipeline:
      1. FTS5 + vector hybrid search → top 20 candidates
      2. Cross-encoder reranks → top 5 returned
    """
    candidates = hybrid_search(query, limit=candidate_limit, db_path=db_path)
...
def main():
    ...
    parser.add_argument("--rerank", action="store_true",
                        help="Apply cross-encoder reranker on top of hybrid search (FTS5 top-20 → reranked top-5). "
                             "Requires: pip3 install sentence-transformers")
    parser.add_argument("--rerank-top", type=int, default=5,
                        help="Number of results to return after reranking (default: 5)")
    ...
    if args.build:
        ...
        if args.rerank:
            print(f"\nSearching (with reranker): '{args.query}'\n{'='*60}")
            results = search_reranked(args.query, candidate_limit=20,
                                      top_k=args.rerank_top, db_path=db_path)
            if not results:
                print("No results found.")
            for i, r in enumerate(results, 1):
                print(f"\n[{i}] {r.get('title', r.get('path', '?'))}")
                print(f"    Path:         brain/{r.get('path', '?')}")
                print(f"    Category:     {r.get('category', '?')}")
                print(f"    Hybrid score: {r.get('hybrid_score', 0):.4f}")
